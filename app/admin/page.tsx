      ...
      <div className="space-y-2">
        {users.map((u:any)=>(
          <div key={u.id} className={`flex justify-between items-center p-3 rounded-2xl bg-zinc-900 border ${u.is_banned?'border-red-500 bg-red-950/20':'border-zinc-800'}`}>
            ...
            <button onClick={()=>toggleBan(u)} className={`px-4 py-2 rounded-full text-xs font-bold ${u.is_banned?'bg-green-500':'bg-yellow-400'} text-black`}>{u.is_banned?'Unban':'Ban'}</button>
          </div>
        ))}
      </div>
    </div>
