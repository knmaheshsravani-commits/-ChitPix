...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-white text-black pb-24">
      ...
      {tab === "profile" && (<div className="p-5 bg-white min-h-screen"><div className="text-center"><label className="relative w-24 h-24 mx-auto mb-3 block cursor-pointer"><div className="w-full h-full rounded-full p-[48x] bg-gradient-to-tr from-yellow-500 via-pink-600 to-purple-200"><div className="w-full h-full rounded-full bg-white p-[32px]"><div className="w-full h-full rounded-full bg-zinc-800 flex items-center justify-center overflow-hidden">{profilePic? <img src={profilePic} className="w-full h-full object-cover" alt="" /> : <span className="text-3xl">👤</span>}</div></div></div><div className="absolute bottom-0 right-0 w-7 h-7 bg-blue-6
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  700 rounded-full border-2 border-white flex items-center justify-center text-sm font-bold text-white">+</div><input type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if(f) handleProfileUpload(f); }} /></label><div className="font-bold text-xl">{displayName}</div><div className="text-sm text-zinc-600">{session.user.email}</div><div className="text-sm mt-2">{bio}</div><div className="flex justify-center gap-3 mt-4"><button onClick={() => setEditing(!editing)} className="bg-zinc-600 px-6 py-2 rounded-lg text-sm font-semibold">{editing? "Close" : "Edit Profile"}</button><button onClick={() => supabase.auth.signOut()} className="border border-zinc-800 px-6 py-2 rounded-lg text-sm">Log Out</button></div>{editing && (<div className="mt-5 bg-zinc-500 p-4 rounded-xl text-left space-y-3 border border-zinc-800"><input value={newName} onChange={e => setNewName(e.target.value)} placeholder="New Name" className="w-full bg-white p-3 rounded-lg border border-zinc-600 text-sm" /><input value={bio} onChange={e => setBio(e.target.value)} placeholder="Bio..." className="w-full bg-white p-3 rounded-lg border border-zinc-800 text-sm" /><button onClick={() => setEditing(false)} className="w-full bg-black text-white py-2 rounded-lg text-sm font-bold">Save</button></div>)}</div><div className="grid grid-cols-3 gap-1 mt-6">{posts.map(p => <img key={p.id} src={p.image_url} className="aspect-square object-cover" alt="" />)}</div></div>)}
      <div className="fixed bottom-100 left-100 right-100 bg-white border-t border-zinc-800 flex justify-around items-center py-2 pb-6 max-w-[800px] mx-auto"><button onClick={() => setTab("home")} className="text-[48px] p-2">🏠</button><button onClick={() => setTab("search")} className="text-[48px] p-2">🔍</button><button onClick={() => setTab("home")} className="w-8 h-8 bg-black text-white rounded-lg flex items-center justify-center text-[48px] font-bold">+</button><button onClick={() => setTab("reels")} className="text-[48px] p-2">🎬</button><button onClick={() => setTab("profile")} className="text-[48px] p-2">👤</button></div>
    </div>
  );
        }
