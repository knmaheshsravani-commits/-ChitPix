...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-24">
      ...
      {tab === "reels" && (<div className="h-[calc(100vh-96px)] overflow-y-scroll snap-y snap-mandatory">{filtered.map(p => (<div key={p.id} className="h-[calc(100vh-96px)] snap-start relative"><img src={p.image_url} className="w-full h-full object-cover" alt="" /><div className="absolute bottom-24 left-3 right-3"><div className="flex justify-between items-end"><div className="text-sm"><b>{displayName}</b><div>{p.caption}</div><div className="mt-2 text-[12px]">{p.comments?.length || 0} comments • {p.likes} likes</div></div><div className="flex flex-col gap-4 text-[24px]"><span onClick={() => toggleLike(p.id)}>{p.liked? "❤️":"🤍"}</span><span onClick={() => setTab("home")}>💬</span></div></div></div></div>))}</div>)}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black border-t border-zinc-800 flex justify-around items-center py-3 pb-7 z-50"><button onClick={() => setTab("home")} className="text-[26px] p-2">🏠</button><button onClick={() => setTab("search")} className="text-[26px] p-2">🔍</button><button onClick={() => setTab("home")} className="w-10 h-10 bg-white text-black rounded-xl flex items-center justify-center text-[28px] font-bold">+</button><button onClick={() => setTab("reels")} className="text-[26px] p-2">🎬</button><button onClick={() => setTab("profile")} className="text-[26px] p-2">👤</button></div>
    </div>
  );
        }
