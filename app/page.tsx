...
export default function Home(){
 ...
 const [posts,setPosts]=useState<any[]>([
  ...
  {id:2,user:"sravani",img:"https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600",likes:890,liked:false,caption:"Beach vibes 🌊 #sunset",comments:[],song:"Summer - Trending"},
  ...
  {id:4,user:"mahesh-07",img:"https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=600",likes:3200,liked:false,caption:"Nature love 🌿",comments:[],song:"Nature"},
 ])
 ...
 const addPost=()=>{ if(!newImg) return alert("Photo select chey!"); setPosts([{id:Date.now(),user:currentUser,img:newImg,likes:0,liked:false,caption:newCap,comments:[],song:"Original - "+currentUser},...posts]); setNewImg(""); setNewCap(""); setShowAdd(false); setTab('home') }
 const addStory=()=>{ if(!newStoryImg) return alert("Story select chey!"); setStories([{id:Date.now(),user:currentUser,img:newStoryImg,time:Date.now(),viewed:false},...stories]); setNewStoryImg(""); setShowStoryAdd(false) }
 ...
 if(tab==='reels'){
  return(
   <div style={{background:'#000',color:'#fff',height:'100vh',overflow:'hidden',position:'relative'}}>
    <div style={{position:'absolute',top:0,left:0,right:0,zIndex:30,display:'flex',justifyContent:'space-between',padding:'15px',background:'linear-gradient(rgba(0,0,0,0.8),transparent)'}}><h2 style={{margin:0}}>Reels 🎬</h2><button onClick={()=>setTab('home')} style={{background:'rgba(255,255,255,0.2)',border:'none',color:'#fff',width:'36px',height:'36px',borderRadius:'50%',fontSize:'18px'}}>✕</button></div>
    <div style={{height:'100vh',overflowY:'scroll',scrollSnapType:'y mandatory'}} onScroll={(e)=>{ const el=e.currentTarget; setReelIndex(Math.round(el.scrollTop/el.clientHeight)) }}>
     {posts.map((p,i)=><div key={p.id} style={{height:'100vh',position:'relative',scrollSnapAlign:'start',scrollSnapStop:'always'}}>
      <img src={p.img} style={{width:'100%',height:'100%',objectFit:'cover'}} alt=""/>
      <div style={{position:'absolute',inset:0,background:'linear-gradient(transparent 30%, rgba(0,0,0,0.85) 90%)'}}/>
      <div style={{position:'absolute',bottom:'30px',left:'15px',right:'80px'}}>
       <div style={{fontWeight:'bold',marginBottom:'6px'}}>@{p.user} {p.user===currentUser && "• Following"}</div>
       <div style={{fontSize:'14px',marginBottom:'8px'}}>{p.caption}</div>
       <div style={{fontSize:'13px',display:'flex',alignItems:'center',gap:'6px'}}>🎵 {p.song} • {p.likes} likes</div>
      </div>
      <div style={{position:'absolute',bottom:'30px',right:'12px',display:'flex',flexDirection:'column',gap:'22px',alignItems:'center'}}>
       <button onClick={()=>setPosts(posts.map(x=>x.id===p.id?{...x,likes:x.liked?x.likes-1:x.likes+1,liked:!x.liked}:x))} style={{background:'none',border:'none',textAlign:'center'}}><div style={{width:'50px',height:'50px',borderRadius:'50%',background:p.liked?'#ff3040':'rgba(255,255,255,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'26px'}}>❤️</div><div style={{color:'#fff',fontSize:'12px',marginTop:'5px',fontWeight:'bold'}}>{p.likes}</div></button>
       <button onClick={()=>setActivePost(p.id)} style={{background:'none',border:'none',textAlign:'center'}}><div style={{width:'50px',height:'50px',borderRadius:'50%',background:'rgba(255,255,255,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'24px'}}>💬</div><div style={{color:'#fff',fontSize:'12px',marginTop:'5px'}}>{p.comments.length}</div></button>
       <button style={{background:'none',border:'none',textAlign:'center'}}><div style={{width:'50px',height:'50px',borderRadius:'50%',background:'rgba(255,255,255,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'24px'}}>🚀</div><div style={{color:'#fff',fontSize:'12px',marginTop:'5px'}}>Share</div></button>
       <div style={{width:'46px',height:'46px',borderRadius:'10px',border:'2px solid #fff',overflow:'hidden',marginTop:'10px'}}><img src={p.img} style={{width:'100%',height:'100%',objectFit:'cover'}} alt=""/></div>
      </div>
     </div>)}
    </div>
    <div style={{position:'absolute',right:'5px',top:'50%',transform:'translateY(-50%)',display:'flex',flexDirection:'column',gap:'5px'}}>{posts.map((_,i)=><div key={i} style={{width:'3px',height:i===reelIndex?'22px':'10px',background:i===reelIndex?'#fff':'rgba(255,255,255,0.4)',borderRadius:'3px'}}/>)}</div>
    {activePost!==null && <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:50,display:'flex',alignItems:'flex-end'}}><div style={{background:'#121212',width:'100%',maxHeight:'60vh',borderRadius:'20px 20px 0 0',borderTop:'1px solid #333',display:'flex',flexDirection:'column'}}><div style={{padding:'15px',borderBottom:'1px solid #222',display:'flex',justifyContent:'space-between'}}><b>Comments</b><button onClick={()=>setActivePost(null)} style={{background:'none',border:'none',color:'#fff',fontSize:'20px'}}>✕</button></div><div style={{flex:1,overflowY:'auto',padding:'15px'}}>{posts.find(x=>x.id===activePost)?.comments.map((c:any,i:number)=><div key={i} style={{marginBottom:'10px'}}><b>@{c.user}</b> {c.text}</div>)}</div><div style={{padding:'12px',display:'flex',gap:'10px'}}><input value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder="Add comment..." style={{flex:1,padding:'12px',borderRadius:'25px',background:'#222',color:'#fff',border:'1px solid #333'}}/><button onClick={addComment} style={{background:'#fff',border:'none',padding:'12px 20px',borderRadius:'25px',fontWeight:'bold'}}>Post</button></div></div></div>}
   </div>
  )
 }
