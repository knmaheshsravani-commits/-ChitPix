    ...
    <div className="min-h-screen bg-white text-black w-full pb-[90px]">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-zinc-300 flex justify-around items-center h-[85px] w-full">
        <button onClick={()=>setTab("home")} className="flex-1 flex justify-center">
          <svg width="42" height="42" viewBox="0 0 24 24" fill={tab==="home"?"black":"none"} stroke="black" strokeWidth="2.5"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        </button>
        <button className="flex-1 flex justify-center">
          <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><polygon points="10 8 16 12 10 16 10 8"/></svg>
        </button>
        <button className="flex-1 flex justify-center">
          <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
        </button>
        <button className="flex-1 flex justify-center">
          <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
        </button>
        <button className="flex-1 flex justify-center">
          <div className="w-[42px] h-[42px] rounded-full bg-black text-white flex items-center justify-center font-bold text-[14px]">M</div>
        </button>
      </div>
    </div>
  );
