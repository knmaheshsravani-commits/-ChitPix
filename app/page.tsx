...
export default function Home(){
 ...
 const addPost=()=>{ if(!newImg) return alert("Photo select chey bro!"); setPosts([{id:Date.now(),user:currentUser,img:newImg,likes:0,liked:false,caption:newCap,comments:[]},...posts]); setNewImg(""); setNewCap(""); setShowAdd(false); setTab('home') }
 ...
 <div style={{background:'#000',color:'#fff',minHeight:'100vh',paddingBottom:'90px'}}>
  ...
  <button onClick={()=>fileRef.current?.click()} style={{width:'100%',padding:'14px',borderRadius:'12px',background:'#222',color:'#fff',border:'2px dashed #555',marginBottom:'10px',fontWeight:'bold'}}>📁 Gallery nunchi Photo Select</button>
  {newImg && <img src={newImg} style={{width:'100%',height:'200px',objectFit:'cover',borderRadius:'12px',marginBottom:'10px'}} alt=""/>}
  <div style={{textAlign:'center',color:'#666',margin:'8px 0',fontSize:'12px'}}>OR</div>
  <input value={newImg} onChange={e=>setNewImg(e.target.value)} placeholder="Image URL kuda petochu..." style={{width:'100%',padding:'12px',borderRadius:'10px',background:'#000',color:'#fff',border:'1px solid #333',marginBottom:'10px',boxSizing:'border-box'}}/>
  <input value={newCap} onChange={e=>setNewCap(e.target.value)} placeholder="Caption..." style={{width:'100%',padding:'12px',borderRadius:'10px',background:'#000',color:'#fff',border:'1px solid #333',marginBottom:'15px',boxSizing:'border-box'}}/><div style={{display:'flex',gap:'10px'}}><button onClick={()=>setShowAdd(false)} style={{flex:1,padding:'12px',borderRadius:'10px',background:'#333',color:'#fff',border:'none'}}>Cancel</button><button onClick={addPost} style={{flex:1,padding:'12px',borderRadius:'10px',background:'#fff',color:'#000',border:'none',fontWeight:'bold'}}>Post 🚀</button></div></div></div>}

  {showStoryAdd && <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.95)',zIndex:110,display:'flex',alignItems:'center',justifyContent:'center',padding:'15px'}}><div style={{background:'#1a1a1a',width:'100%',maxWidth:'400px',borderRadius:'16px',padding:'20px',border:'1px solid #333'}}><h3 style={{marginTop:0}}>Add Story 🔥</h3>
  <input type="file" ref={storyFileRef} accept="image/*" hidden onChange={(e)=>handleFile(e,setNewStoryImg)}/>
  <button onClick={()=>storyFileRef.current?.click()} style={{width:'100%',padding:'14px',borderRadius:'12px',background:'#222',color:'#fff',border:'2px dashed #555',marginBottom:'10px',fontWeight:'bold'}}>📁 Gallery nunchi Story Select</button>
  {newStoryImg && <img src={newStoryImg} style={{width:'100%',height:'200px',objectFit:'cover',borderRadius:'12px',marginBottom:'10px'}} alt=""/>}
  <input value={newStoryImg} onChange={e=>setNewStoryImg(e.target.value)} placeholder="URL kuda petochu..." style={{width:'100%',padding:'12px',borderRadius:'10px',background:'#000',color:'#fff',border:'1px solid #333',marginBottom:'15px',boxSizing:'border-box'}}/><div style={{display:'flex',gap:'10px'}}><button onClick={()=>setShowStoryAdd(false)} style={{flex:1,padding:'12px',borderRadius:'10px',background:'#333',color:'#fff',border:'none'}}>Cancel</button><button onClick={addStory} style={{flex:1,padding:'12px',borderRadius:'10px',background:'linear-gradient(135deg,#ff7a00,#ff3ca0)',color:'#fff',border:'none',fontWeight:'bold'}}>Add Story 🚀</button></div></div></div>}
