...
<div className="max-w-[470px] mx-auto pb-32">
{tab==="home" && filteredPosts.map((p:any)=><div key={p.id} className="border-b border-zinc-800 pb-3"><div className="flex items-center gap-2 p-3"><img src={getImg(p)} className="w-8 h-8 rounded-full object-cover bg-zinc-800"/><span className="font-semibold text-[13px]">{p.username||"mahesh123"}</span></div><div className="w-full aspect-square bg-zinc-900"><img src={getImg(p)} className="w-full h-full object-cover"/></div><div className="flex gap-5 p-4 items-center"><button onClick={()=>likePost(p.id)}><svg style={{width:'32px',height:'32px'}} fill={liked.has(p.id)?"white":"none"} stroke="white" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg></button><button><svg style={{width:'32px',height:'32px'}} fill="none" stroke="white" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg></button><button><svg style={{width:'32px',height:'32px'}} fill="none" stroke="white" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M22 2L11 13"/><path d="M22 2l-7 20-4-9-9-4 20-7z"/></svg></button><button className="ml-auto"><svg style={{width:'32px',height:'32px'}} fill="none" stroke="white" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg></button></div><div className="px-3 text-[13px]"><span className="font-bold">{24 + (liked.has(p.id)?1:0)} likes</span><div className="mt-1"><span className="font-bold mr-2">{p.username||"mahesh123"}</span>{p.caption}</div></div></div>)}
...
{tab==="search" && (<div className="p-3"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search" className="w-full bg-zinc-800 rounded-lg px-4 py-3 text-sm outline-none"/><div className="grid grid-cols-3 gap-[2px] mt-3">{filteredPosts.map(p=><div key={p.id} className="aspect-square bg-zinc-900"><img src={getImg(p)} className="w-full h-full object-cover"/></div>)}</div></div>)}
