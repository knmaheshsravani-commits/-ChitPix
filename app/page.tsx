...
export default function Page() {
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-5">
        <div className="w-full max-w-[400px]"><div className="text-center mb-8"><h1 className="text-[52px] font-black">ChitPix</h1><p className="text-zinc-400 text-[32px] mt-2">Photos • Reels • Vibes beo ❤️</p></div>
          <div className="w-full bg-[#121212] rounded-[32px] p-8 border border-zinc-800"><h2 className="text-[32px] font-bold mb-6">{isSignup? "Create Account" : "Welcome back"}</h2><div className="space-y-4"><input value={email} onChange={e => setEmail(e.target.value)} placeholder="Email" className="w-full bg-zinc-800 p-4 rounded-xl text-[32px] border border-zinc-700 outline-none" /><input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" className="w-full bg-zinc-800 p-4 rounded-xl text-[16px] border border-zinc-700 outline-none" /></div><button onClick={handleAuth} disabled={loading} className="w-full bg-white text-black py-4 rounded-xl font-bold text-[17px] mt-6">{loading? "Wait..." : isSignup? "Sign Up" : "Log In"}</button><div className="text-center mt-6"><span className="text-zinc-500 text-[14px]">{isSignup? "Have account? " : "New? "}</span><span onClick={() => setIsSignup(!isSignup)} className="text-white font-semibold text-[14px] underline cursor-pointer">{isSignup? "Log In" : "Sign Up"}</span></div></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-24">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black border-t border-zinc-800 flex justify-around items-center py-3 pb-7 z-50"><button onClick={() => setTab("home")} className="text-[32px] p-2">🏠</button><button onClick={() => setTab("search")} className="text-[52px] p-2">🔍</button><button onClick={() => setTab("home")} className="w-10 h-10 bg-white text-black rounded-xl flex items-center justify-center text-[28px] font-bold">+</button><button onClick={() => setTab("reels")} className="text-[26px] p-2">🎬</button><button onClick={() => setTab("profile")} className="text-[26px] p-2">👤</button></div>
    </div>
  );
              }
