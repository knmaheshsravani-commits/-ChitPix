              ...
              <div key={p.id} style={{ border: '1px solid #333', borderRadius: '12px', marginBottom: '20px', background: '#111', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '12px' }}>
                  <div style={{ display: 'flex', gap: '15px', marginBottom: '8px' }}>
                    {p.likes}</button><button onClick={()=>handleShare(p)} style={{border:'none',background:'none',fontSize:'22px'}}>🚀</button><button onClick={()=>setShowCmt({...showCmt, [p.id]:!showCmt[p.id]})}
