...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-white text-black pb-24">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-zinc-800 flex justify-around items-center py-2 pb-6 max-w-[800px] mx-auto"><button onClick={() => setTab("home")} className="text-[52px] p-2">🏠</button><button onClick={() => setTab("search")} className="text-[52px] p-2">🔍</button><button onClick={() => setTab("home")} className="w-8 h-8 bg-black text-white rounded-lg flex items-center justify-center text-[32px] font-bold">+</button><button onClick={() => setTab("reels")} className="text-[52px] p-2">🎬</button><button onClick={() => setTab("profile")} className="text-[52px] p-2">👤</button></div>
    </div>
  );
        }
