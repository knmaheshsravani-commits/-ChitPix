...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-white text-black pb-[90px]">
      ...
      {tab==="search" && (
        <div className="p-4">
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search user..." className="w-full border-2 p-3 rounded-full outline-none focus:border-black"/>
          <div className="mt-4 grid grid-cols-2 gap-2">
            {filtered.map(p=><img key={p.id} src={p.img} className="w-full aspect-square object-cover rounded"/>)}
          </div>
          {filtered.length===0 && <p className="text-center mt-10 text-gray-400">No user found</p>}
        </div>
      )}
      ...
      {tab==="profile" && (
        <div className="p-4">
          <div className="flex gap-5 items-center">
            <img src="https://picsum.photos/200/200?random=10" className="w-20 h-20 rounded-full"/>
            <div className="flex gap-6"><div className="text-center"><p className="font-bold">12</p><p className="text-[13px]">Posts</p></div><div className="text-center"><p className="font-bold">1.2K</p><p className="text-[13px]">Followers</p></div><div className="text-center"><p className="font-bold">300</p><p className="text-[13px]">Following</p></div></div>
          </div>
          <p className="font-bold mt-3">_sankar_001</p><p className="text-[14px]">Devanhalli | ChitPix Creator</p>
          <div className="grid grid-cols-3 gap-1 mt-4">{posts.map(p=><img key={p.id} src={p.img} className="aspect-square object-cover"/> )}</div>
        </div>
      )}

      {showComments && (
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-end" onClick={()=>setShowComments(false)}>
          <div className="bg-white w-full rounded-t-[20px] p-4 h-[50vh]"><p className="font-bold text-center border-b pb-2">Comments</p><p className="mt-3"><b>rxtagur</b> Mass bro 🔥</p><p className="mt-2"><b>akhilesh</b> Super pic!</p><input placeholder="Add a comment..." className="w-full border rounded-full p-3 mt-4"/></div>
        </div>
      )}

      {/* MEGA BIG BOTTOM NAV - WORKING */}
