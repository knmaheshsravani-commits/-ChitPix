...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-white text-black pb-24">
      {tab === "home" && (<div className="max-w-[800px] mx-auto"><div className="flex justify-between items-center p-3 border-b border-zinc-600 bg-white sticky top-0 z-10"><span className="font-bold text-xl">ChitPix</span><span className="text-sm font-medium">{displayName}</span></div><div className="p-3 bg-white"><input value={text} onChange={e => setText(e.target.value)} placeholder="What's on your mind?" className="w-full bg-zinc-100 border border-zinc-600 p-3 rounded-lg text-sm outline-none" /><input value={url} onChange={e => setUrl(e.target.value)} placeholder="Paste image URL..." className="w-full bg-zinc-600 border border-zinc-600 p-3 rounded-lg text-sm mt-2 outline-none" /><label className="mt-2 flex items-center justify-center rounded-lg border border-dashed border-zinc-400 p-3 text-sm cursor-pointer bg-zinc-50">{uploading? "Uploading..." : "📷 Gallery nundi Photo"}<input type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) handleFileUpload(f); }} /></label><button onClick={addPost} className="w-full bg-blue-600 text-white py-2.5 rounded-lg font-semibold text-sm mt-3">Post</button></div>{posts.map(p => (<div key={p.id} className="border-b border-zinc-400 bg-white"><div className="p-3 font-bold text-sm flex items-center gap-2"><div className="w-8 h-8 rounded-full p-[8px] bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600"><div className="w-full h-full rounded-full bg-white border-2 border-white overflow-hidden">{profilePic && <img src={profilePic} className="w-full h-full object-cover" alt="" />}</div></div>{displayName}</div><img src={p.image_url} onDoubleClick={() => { if(!p.liked) toggleLike(p.id)}} className="w-full aspect-[4/5] object-cover bg-zinc-100" alt="" />
