...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-[80px]">
      ...
      {/* BOTTOM NAV - GIANT SIZE */}
<div className="fixed bottom-0 left-0 right-0 bg-black border-t border-zinc-700 flex justify-around items-center py-4 max-w-[470px] mx-auto h-[75px]">
  <button onClick={()=>setTab("home")} className="p-2">
    <svg width="36" height="36" viewBox="0 0 24 24" fill={tab==="home"?"white":"none"} stroke="white" strokeWidth="2.2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
  </button>
  <button onClick={()=>setTab("search")} className="p-2">
    <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
  </button>
  <button onClick={()=>setTab("reels")} className="p-2">
    <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2"><polygon points="23 7 13 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
  </button>
  <button onClick={()=>setTab("plus")} className="p-2">
    <svg width="38" height="38" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2"><rect x="3" y="3" width="18" height="18" rx="5"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
  </button>
  <button onClick={()=>setTab("profile")} className="p-2">
    <div className={`w-9 h-9 rounded-full border-[2.5px] ${tab==="profile"?"border-white":"border-zinc-500"} bg-zinc-700 flex items-center justify-center text-[16px]`}>👤</div>
  </button>
</div>
