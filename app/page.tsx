...
export default function Home(){
 ...
 const [posts,setPosts]=useState<any[]>([
  ...
  {id:3,user:"mahesh-07",img:"https://images.unsplash.com/photo-1523712999610-f77fbcfc3843?w=600",likes:20,liked:false,caption:"Golden hour ✨",comments:[{user:"mahesh-07",text:"Tq guys!"}]},
 ])
   ...
   <div style={{background:'#000',color:'#fff',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
    <div style={{width:'100%',maxWidth:'360px',textAlign:'center'}}>
     ...
     <div style={{background:'#111',borderRadius:'16px',padding:'20px',border:'1px solid #222'}}>
      ...
      <button onClick={handleAuth} style={{width:'100%',padding:'14px',borderRadius:'12px',background:'linear-gradient(135deg,#ff7a00,#ff3ca0)',color:'#fff',border:'none',fontWeight:'bold'}}>Login to ChitPix 🚀</button>
     </div>
    </div>
   </div>
  )
 ...
 <div style={{background:'#000',color:'#fff',minHeight:'100vh',paddingBottom:'90px'}}>
  ...
  {activePost!==null && (
   <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:90,display:'flex',alignItems:'flex-end'}}>
    <div style={{background:'#121212',width:'100%',maxHeight:'70vh',borderRadius:'20px 20px 0 0',borderTop:'1px solid #333',display:'flex',flexDirection:'column'}}>
     <div style={{padding:'15px',borderBottom:'1px solid #222',display:'flex',justifyContent:'space-between',alignItems:'center'}}><b>Comments</b><button onClick={()=>setActivePost(null)} style={{background:'none',border:'none',color:'#fff',fontSize:'20px'}}>✕</button></div>
     <div style={{flex:1,overflowY:'auto',padding:'15px'}}>
      {posts.find(p=>p.id===activePost)?.comments.map((c:any,i:number)=><div key={i} style={{marginBottom:'12px'}}><b>@{c.user}</b> <span style={{marginLeft:'6px'}}>{c.text}</span></div>)}
      {posts.find(p=>p.id===activePost)?.comments.length===0 && <p style={{color:'#666',textAlign:'center'}}>No comments yet. Be first! 👇</p>}
     </div>
     <div style={{padding:'12px',borderTop:'1px solid #222',display:'flex',gap:'10px'}}>
      <input value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder="Add a comment..." style={{flex:1,padding:'12px',borderRadius:'25px',background:'#222',color:'#fff',border:'1px solid #333'}}/>
  ...
  {showAdd && (
   <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.9)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center',padding:'15px'}}>
    <div style={{background:'#1a1a1a',width:'100%',maxWidth:'400px',borderRadius:'16px',padding:'20px',border:'1px solid #333'}}>
     <h3 style={{margin:'0 0 15px'}}>New Post 📸</h3>
  ...
  <div style={{position:'fixed',bottom:0,left:0,right:0,background:'#000',borderTop:'1px solid #222',display:'flex',justifyContent:'space-around',alignItems:'center',padding:'12px 0',zIndex:50}}>
   <button onClick={()=>setTab('home')} style={{background:'none',border:'none',fontSize:'28px'}}>🏠</button>
   <button style={{background:'none',border:'none',fontSize:'28px'}}>🔍</button>
   <button onClick={()=>setShowAdd(true)} style={{background:'linear-gradient(135deg,#ff7a00,#ff3ca0)',border:'none',width:'52px',height:'52px',borderRadius:'50%',fontSize:'32px',color:'#fff',marginTop:'-15px',boxShadow:'0 0 20px rgba(255,60,160,0.6)'}}>+</button>
   <button style={{background:'none',border:'none',fontSize:'28px'}}>🎬</button>
   <button style={{background:'none',border:'none',fontSize:'28px'}}>👤</button>
  </div>
 </div>
