...
export default function Home(){
 ...
 const [posts,setPosts]=useState([
  {id:1,user:"mahesh-07",img:"https://images.unsplash.com/photo-1551632811-561732d1e306?w=600",likes:12,liked:false,caption:"First post 🔥"},
  {id:2,user:"sravani",img:"https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600",likes:5,liked:false,caption:"Beach day 🌊"},
 ...
 const addPost=()=>{
  if(!newImg) return alert("Photo URL petti bro!")
 ...
 <div style={{background:'#000',color:'#fff',minHeight:'100vh',paddingBottom:'80px'}}>
  ...
  <div style={{display:'flex',justifyContent:'space-between',padding:'12px 15px',borderBottom:'1px solid #222',position:'sticky',top:0,background:'#000',zIndex:20,alignItems:'center'}}>
   <div style={{display:'flex',gap:'10px',alignItems:'center'}}>
    <div style={{width:'42px',height:'42px',borderRadius:'12px',background:'linear-gradient(135deg,#ff7a00,#ff3ca0,#8a2be2,#3a5bff)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'22px'}}>📸</div>
    <h1 style={{margin:0,fontSize:'26px',fontWeight:900}}><span>Chit</span><span style={{color:'#ff7a00'}}>pix</span></h1>
   </div>
  ...
  <div style={{maxWidth:'500px',margin:'0 auto'}}>
   {tab==='home' && posts.map(p=>(
    <div key={p.id} style={{borderBottom:'8px solid #111'}}>
     ...
     <img src={p.img} style={{width:'100%',height:'420px',objectFit:'cover',background:'#111'}} />
     <div style={{display:'flex',gap:'20px',padding:'12px'}}>
      <button onClick={()=>setPosts(posts.map(x=>x.id===p.id?{...x,likes:x.liked?x.likes-1:x.likes+1,liked:!x.liked}:x))} style={{background:'none',border:'none',color:p.liked?'#ff3040':'#fff',fontSize:'20px',fontWeight:'bold'}}>❤️ {p.likes}</button>
   ...
   {tab==='reels' && <div>{posts.map(p=><div key={p.id} style={{position:'relative',height:'80vh',marginBottom:'5px'}}><img src={p.img} style={{width:'100%',height:'100%',objectFit:'cover'}}/><div style={{position:'absolute',bottom:'20px',left:'10px'}}>🎬 @{p.user} - {p.caption}</div></div>)}</div>}
  ...
  {showAdd && (
   <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center',padding:'15px'}}>
    <div style={{background:'#1a1a1a',width:'100%',maxWidth:'400px',borderRadius:'16px',padding:'20px',border:'1px solid #333'}}>
     ...
     <input value={newImg} onChange={e=>setNewImg(e.target.value)} placeholder="Paste Image URL (unsplash / any link)" style={{width:'100%',padding:'12px',borderRadius:'10px',background:'#000',color:'#fff',border:'1px solid #333',marginBottom:'10px'}}/>
     <input value={newCap} onChange={e=>setNewCap(e.target.value)} placeholder="Caption... ex: Beach vibe 🌊" style={{width:'100%',padding:'12px',borderRadius:'10px',background:'#000',color:'#fff',border:'1px solid #333',marginBottom:'15px'}}/>
     {newImg && <img src={newImg} style={{width:'100%',height:'200px',objectFit:'cover',borderRadius:'10px',marginBottom:'15px'}} />}
     ...
     <p style={{fontSize:'11px',color:'#666',marginTop:'10px',textAlign:'center'}}>Tip: Google lo image -> Share -> Copy link paste chey</p>
    </div>
   </div>
  )}
  ...
  <div style={{position:'fixed',bottom:0,left:0,right:0,background:'#000',borderTop:'1px solid #222',display:'flex',justifyContent:'space-around',padding:'14px 0',zIndex:50}}>
   <button onClick={()=>setTab('home')} style={{background:'none',border:'none',fontSize:'26px',opacity:tab==='home'?1:0.3}}>🏠</button>
   <button onClick={()=>setTab('search')} style={{background:'none',border:'none',fontSize:'26px',opacity:tab==='search'?1:0.3}}>🔍</button>
   <button onClick={()=>setShowAdd(true)} style={{background:'linear-gradient(135deg,#ff7a00,#ff3ca0)',border:'none',width:'48px',height:'48px',borderRadius:'50%',fontSize:'30px',color:'#fff',marginTop:'-10px',boxShadow:'0 0 15px #ff3ca0'}}> + </button>
   <button onClick={()=>setTab('reels')} style={{background:'none',border:'none',fontSize:'26px',opacity:tab==='reels'?1:0.3}}>🎬</button>
   <button onClick={()=>setTab('profile')} style={{background:'none',border:'none',fontSize:'26px',opacity:tab==='profile'?1:0.3}}>👤</button>
  </div>
 </div>
 )
               }
