...
export default function ChitPix(){
  ...
  async function postNow(){
    ...
    await supabase.from("posts").insert({image_url:publicUrl, caption, user_id:user.id})
    setCaption(""); setFile(null); setTab("home"); getPosts()
  }

  if(!user) return(
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <div className="w-full max-w-sm border border-zinc-800 p-6 rounded-2xl">
        <h1 className="text-3xl font-bold text-center mb-6">ChitPix</h1>
        <input placeholder="Email" className="w-full p-3 mb-3 bg-zinc-900 border border-zinc-800 rounded-xl" onChange={e=>setEmail(e.target.value)}/>
        <input placeholder="Password" type="password" className="w-full p-3 mb-3 bg-zinc-900 border border-zinc-800 rounded-xl" onChange={e=>setPass(e.target.value)}/>
        <button onClick={login} className="w-full bg-white text-black p-3 rounded-xl font-bold mb-2">Log In</button>
        <button onClick={signup} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800">Sign Up</button>
      </div>
    </div>
  )
  ...
  return(
    <div className="min-h-screen bg-black text-white pb-28">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black border-t-2 border-zinc-800 flex justify-around py-4">
        ...
        <button onClick={()=>setTab("create")} className="bg-white text-black p-2 rounded-full"><Plus size={24}/></button>
