...
export default function Page() {
  ...
  const [posts, setPosts] = useState<any[]>([
    { id: 1, image_url: "https://picsum.photos/seed/1/800/1000", caption: "First post beo!", likes: 12, liked: false },
  ]);
    ...
    <div className="min-h-screen bg-black text-white pb-24">
      {tab === "home" && (
        <div className="max-w-[600px] mx-auto">
          ...
          {posts.map(p => (
            <div key={p.id} className="border-b border-zinc-800">
              ...
              <div className="flex justify-between p-3 text-[52px]"><div className="flex gap-4"><span onClick={() => toggleLike(p.id)} className="cursor-pointer">{p.liked? "❤️" : "🤍"}</span><span>💬</span><span>↗️</span></div><span>🔖</span></div>
              <div className="px-3 text-sm font-semibold">{p.likes} likes</div>
              <div className="px-3 pb-3 text-[52px]"><b>{myName}</b> {p.caption}</div>
            </div>
                {tab === "search" && (
        <div className="max-w-[470px] mx-auto">
          ...
          {search === ""? (
            ...
            <>
              <div className="px-3 py-2 text-[32px] text-zinc-500">{filtered.length} results for "{search}"</div>
              {filtered.length === 0? (
                <div className="text-center mt-20 text-zinc-600"><div className="text-4xl mb-2">😕</div><div className="text-[32px]">No results found</div></div>
              ) : (
                <div className="grid grid-cols-3 gap-[8px]">{filtered.map(p => <img key={p.id} src={p.image_url} className="aspect-square object-cover" alt="" />)}</div>
              )}
            </>
          )}
        </div>
      ...
      {tab === "reels" && (<div className="h-[calc(100vh-100px)] overflow-y-scroll snap-y snap-mandatory"><>{filtered.map(p => (<div key={p.id} className="h-[calc(100vh-96px)] snap-start relative"><img src={p.image_url} className="w-full h-full object-cover" alt="" /><div className="absolute bottom-20 left-3 text-sm"><b>{myName}</b><div>{p.caption}</div></div></div>))}</></div>)}
        ...
        <div className="p-5 text-center">
          ...
          <div className="text-sm text-zinc-500 mb-4">{session.user.email}</div>
          <button onClick={() => supabase.auth.signOut()} className="border border-zinc-800 px-6 py-2 rounded-lg text-sm">Log Out</button>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black border-t border-zinc-800 flex justify-around items-center py-3 pb-7 z-50">
        ...
        <button onClick={() => setTab("home")} className="w-10 h-10 bg-white text-black rounded-xl flex items-center justify-center text-[52px] font-bold">+</button>
