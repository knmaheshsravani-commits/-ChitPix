...
export default function Home(){
 ...
 <div style={{background:'#000',color:'#fff',minHeight:'100vh',paddingBottom:'90px'}}>
  ...
  {tab==='home' && <>
   <div style={{display:'flex',gap:'12px',padding:'12px 10px',overflowX:'auto',borderBottom:'1px solid #111',scrollbarWidth:'none'}}>
  ...
  {activeStory!==null && validStories[activeStory] && (
   <div style={{position:'fixed',inset:0,background:'#000',zIndex:200,display:'flex',flexDirection:'column'}}>
    <div style={{display:'flex',gap:'3px',padding:'8px'}}>{validStories.map((_,idx)=><div key={idx} style={{flex:1,height:'3px',background: idx<activeStory!? '#fff' : idx===activeStory? '#ff3ca0' : 'rgba(255,255,255,0.3)',borderRadius:'2px'}}></div>)}</div>
    <div style={{display:'flex',justifyContent:'space-between',padding:'10px 15px',alignItems:'center'}}><span style={{fontWeight:'bold'}}>🔥 @{validStories[activeStory].user}</span><button onClick={()=>setActiveStory(null)} style={{background:'none',border:'none',color:'#fff',fontSize:'24px'}}>✕</button></div>
    <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',position:'relative'}}><img src={validStories[activeStory].img} style={{maxWidth:'100%',maxHeight:'80vh',objectFit:'contain'}} alt=""/><button onClick={()=>setActiveStory(activeStory>0?activeStory-1:null)} style={{position:'absolute',left:0,top:0,bottom:0,width:'30%',background:'none',border:'none'}}></button><button onClick={()=>setActiveStory(activeStory+1 < validStories.length? activeStory+1 : null)} style={{position:'absolute',right:0,top:0,bottom:0,width:'70%',background:'none',border:'none'}}></button></div>
   </div>
  )}
  ...
  {showStoryAdd && <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.9)',zIndex:110,display:'flex',alignItems:'center',justifyContent:'center',padding:'15px'}}><div style={{background:'#1a1a1a',width:'100%',maxWidth:'400px',borderRadius:'16px',padding:'20px',border:'1px solid #333'}}><h3>Add Story 🔥 (24h)</h3><input value={newStoryImg} onChange={e=>setNewStoryImg(e.target.value)} placeholder="Story Image URL" style={{width:'100%',padding:'12px',borderRadius:'10px',background:'#000',color:'#fff',border:'1px solid #333',marginBottom:'15px',boxSizing:'border-box'}}/><div style={{display:'flex',gap:'10px'}}><button onClick={()=>setShowStoryAdd(false)} style={{flex:1,padding:'12px',borderRadius:'10px',background:'#333',color:'#fff',border:'none'}}>Cancel</button><button onClick={addStory} style={{flex:1,padding:'12px',borderRadius:'10px',background:'linear-gradient(135deg,#ff7a00,#ff3ca0)',color:'#fff',border:'none',fontWeight:'bold'}}>Add Story 🚀</button></div></div></div>}
